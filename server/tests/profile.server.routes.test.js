...
describe('Item CRUD tests', function() {
  ...
  it('should be able to save profile', function(done) {
  ...
  it('should be able to read profile', function(done) {
    Profile.findOne({Name: 'Marco'}, function(err, profile) {
      if(err) {
        console.log(err);
      } else {
	id2=profile._id;
	console.log('profileId:' +profile._id);
	agent.get('/api/profile/'+ profile._id)
          .expect(200)
          .end(function(err, res) {
            should.not.exist(err);
            should.exist(res);
		...
		if(err) {
			...
			agent.get('/api/profile/' + profile._id)
        		  ...
        		  .end(function(err, res) {
        		    ...
        		    should.exist(res.body._id);
        		    res.body.Name.should.equal('Marco');                                                          
        		    console.log('Response: /n'+ res.body);
			    profile = res.body;
	    		});
			console.log(profile);
        		...
        		console.log(profile);     
        		agent.put('/api/profile/' + profile._id)
        		  .send(profile)
        		  .expect(200)
        		  .end(function(err, res) {
        		    should.not.exist(err);
